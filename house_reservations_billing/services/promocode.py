...
def check_availability(
    ...
    if promo_code.expiration_datetime and now() > promo_code.expiration_datetime:
        raise exceptions.ValidationError(f"Промокод истек {promo_code.expiration_datetime.strftime('%d-%m-%Y %H:%M')}")
    ...
    if promo_code.client and client:  # когда происходит запрос цены бронирования информация приходит без клиента
        if promo_code.client != client:
            raise exceptions.ValidationError(f"Промокод принадлежит другому пользователю (идентификация по email)")
    ...
    if bill.id and promo_code.bills.filter(id=bill.id):  # если чек уже сохранен в бд с этим промокодом
        ...
        raise exceptions.ValidationError('Промокод уже был использован максимальное количество раз')

    # Check bill value
    if value < promo_code.minimal_bill_value:
        raise exceptions.ValidationError(f'Промокод доступен при сумме в чеке от {promo_code.minimal_bill_value}')
